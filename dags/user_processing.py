...
def _store_user():
    ...
    hook.copy_expert(
        ...
        sql="""
        COPY users (firstname, lastname, country, username, password, email) 
        FROM stdin WITH DELIMITER as ',' 
        """,
        filename='/tmp/processed_user.csv'
    )

with DAG('user_processing',# Dag name
         start_date = datetime(2023,1,1),
        schedule_interval='*/30 * * * *', # this is the cron operation which runs every 30 mins
